...
from jetengine import Document, StringField, BooleanField, ListField, DESCENDING, DateTimeField, IntField, Aggregation
...
class TestAggregation(AsyncTestCase):
    ...
    def add_cities(self):
        ...
        for i in range(500):
            ...
            cities.append(City(city=city_name, state=state_name, pop=randint(10000, 50000)))
    ...
    def test_can_aggregate_number_of_documents(self):
        result = yield from User.objects.aggregate.group_by(
            User.email, Aggregation.avg(User.number_of_documents, alias="number_of_documents")
        ).fetch()
    ...
    def test_can_aggregate_with_unwind(self):
        result = (
            yield from User.objects.aggregate.unwind(User.list_items)
            .group_by(
                User.email, User.list_items, Aggregation.avg(User.number_of_documents, alias="number_of_documents")
            )
            .fetch()
        )
    ...
    def test_can_aggregate_with_sorting(self):
        result = yield from User.objects.aggregate.order_by(User.number_of_documents, DESCENDING).fetch()
    ...
    def test_can_average_city_population(self):
        results = (
            yield from City.objects.aggregate.group_by(
                City.state, City.city, Aggregation.sum(City.pop, alias="total_pop")
            )
            .group_by(City.state, Aggregation.avg("total_pop", alias="avg_pop"))
            .fetch()
        )
